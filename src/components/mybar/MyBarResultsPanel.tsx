...
interface MyBarResultsPanelProps {
...
export default function MyBarResultsPanel({
  myBarIngredients,
  recipesICanMake,
    ...
    <div className="space-y-6 min-w-0">
          ...
          {recipesICanMake.length > 6 && (
            <div className="text-center mt-4">
              <Button
                ...
                className="text-sm"
              >
                {showAllRecipes ? "Show less" : `Show more (${recipesICanMake.length - 6} more)`}
              </Button>
            </div>
          )}
        </div>
      ...
      {recipesNeedingOneIngredient.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-lg font-semibold text-pure-white flex items-center gap-2">
            ...
            Missing One Ingredient ({recipesNeedingOneIngredient.length})
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
      ...
      {myBarIngredients.length > 0 && recipesICanMake.length === 0 && recipesNeedingOneIngredient.length === 0 && (
        <div className="text-center py-12">
          <div className="text-5xl font-bold text-soft-gray">0</div>
          <div className="text-soft-gray mt-2">Add a few more ingredients to unlock cocktails</div>
        </div>
      )}
    </div>
  );
